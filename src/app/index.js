...
// Called when the background sends a sync command to update view model
  ...
  await client.start(); // this one is slow because it has to load the file (250ms)
  //console.log(Date.now());
  // this is running in like 20ms *total* for 50 requests with an object
  // faster if i remove console.log from the worker.
  //for ( let i = 0; i < 50; i++ ) {
  //    let resp = await client.request('test', { i, vm: window.view });
  //console.log(resp);
  //}
  //console.log(Date.now());
})();
...
ViewModel.currentCity = ViewModel.cities[0];
